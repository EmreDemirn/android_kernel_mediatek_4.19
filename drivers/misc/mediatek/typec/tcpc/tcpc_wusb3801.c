...
#ifdef __TEST_CC_PATCH__
/* defined in mtk_chg_type_det.c (matches 4.14 yey layout) */
...
static int wusb3801_init_alert(struct tcpc_device *tcpc)
{
	...
	if (chip->irq <= 0) {
		pr_err("%s gpio to irq fail, chip->irq(%d)\n",
						__func__, chip->irq);
		/* propagate EPROBE_DEFER: irq domain may not be ready yet */
...
static int wusb3801_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
	...
	pr_info("%s enter \n", __func__);
	/* Check initial attach state only after tcpc_typec_init() has run:
	 * it resets the state machine to Unattached.SNK, so any attach
	 * reported before this point (e.g. cable already plugged at boot)
	 * would be wiped and never re-reported (no IRQ edge).
	 */
				...
				const struct i2c_device_id *id)
{
	...
	/* first_check_typec_work is scheduled from wusb3801_tcpc_init()
	 * (after tcpc_typec_init resets the state machine), not here:
	 * running it earlier gets its attach report wiped by typec_init,
	 * and scheduling before the last failure point is a use-after-free.
	 */
	pr_info("%s probe OK!\n", __func__);
	return 0;
